...
import { extname, resolve as nodePathResolve } from 'node:path';
...
export const load = async (url, context, defaultLoad) => {
  ...
  if (/^file:\/{3}.*\/bin\//.test(url) && !extname(url)) {
...
export const resolve = async (specifier, context, defaultResolve) => {
   ...
   */
  const result = loadConfig(nodePathResolve('.'));
  ...
  if (!isFileUrl && match) specifier = pathToFileURL(specifier).toString();
